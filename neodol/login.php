 ...
 * Version: 2.1.2 - SECURITY: Session Matching, PDO Migration, CSRF
 */
require_once('includes/db.php');

// --- SESSION INITIALIZATION (Matching index.php) ---
if (session_status() === PHP_SESSION_NONE) {
    session_name('NEODOL_SESSION'); // Muss exakt wie in der index.php sein
    session_start();
}
...
try {
    ...
    error_log("Login Security Table missing: " . $e->getMessage());
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($error)) {
    ...
    if ($u && aldhran_verify($pass_raw, $u['password'])) {
        ...
        // Session-Variablen setzen (NEODOL_SESSION)
