...
<div class="home-forum-widget" style="margin-bottom: 40px;">
    ...
    <?php if (!empty($latest_threads)): ?>
        <div style="display: grid; gap: 10px;">
            <?php 
            // Prepared Statement außerhalb der Schleife vorbereiten (Performance!)
            $stmt_realm = $db->prepare("SELECT DISTINCT Realm FROM dolcharacters WHERE AccountName = ? LIMIT 1");
            
            foreach ($latest_threads as $lt): 
                // Realm-Farbe via PDO holen
                $stmt_realm->execute([$lt['username']]);
                $r_data = $stmt_realm->fetch();
                
                $r_color = '#333'; // Default
                if($r_data) {
                    $realm = (int)$r_data['Realm'];
                    if($realm === 1) $r_color = '#F52727'; // Albion
                    elseif($realm === 2) $r_color = '#275BF5'; // Midgard
                    elseif($realm === 3) $r_color = '#27F565'; // Hibernia
                }
            ?>
                <div class="quick-card" onclick="window.location.href='?p=viewthread&id=<?php echo (int)$lt['id']; ?>'" 
                     style="display: flex; align-items: center; justify-content: space-between; padding: 12px 20px; border-left: 3px solid <?php echo $r_color; ?>; cursor: pointer; background: rgba(0,0,0,0.2); transition: 0.2s;">
                    
                    <div style="display: flex; align-items: center; gap: 15px;">
                        <div style="width: 35px; height: 35px; border: 1px solid #222; overflow: hidden; background: #000;">
                            <?php if(!empty($lt['avatar_url'])): ?>
                                <img src="<?php echo h($lt['avatar_url']); ?>" style="width:100%; height:100%; object-fit:cover;">
                            <?php else: ?>
                                <i class="fas fa-user-circle" style="font-size: 33px; color: #111;"></i>
                            <?php endif; ?>
                        </div>
                        <div>
                            <div style="color: #eee; font-weight: bold; font-size: 0.95em;"><?php echo h($lt['title']); ?></div>
                            <div style="font-size: 0.75em; color: #555;">
                                by <span style="color: var(--glow-blue);"><?php echo h($lt['username']); ?></span> in <?php echo h($lt['board_title']); ?>
                            </div>
                        </div>
                    </div>
