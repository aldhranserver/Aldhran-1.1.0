<?php 
/**
 * UM VIEW - Aldhran Freeshard
 * Version: 1.4.1 - Spike Forum aware
 */
...
<script>
    ...
    searchInput.addEventListener('input', function() {
        if(this.value.length > 1) {
            ...
            params.append('um_ajax_search', this.value);
            params.append('can_edit', '1');
            fetch('modules/um_sync_worker.php', { method: 'POST', body: params })
            .then(res => res.text()).then(data => { resultsOverlay.innerHTML = data; resultsOverlay.style.display = 'block'; });
    ...
    function loadCategory(cat) {
        ...
        const params = new URLSearchParams();
        params.append('um_load_cat', cat);
        params.append('can_edit', '1');
        fetch('modules/um_sync_worker.php', { method: 'POST', body: params })
        .then(res => res.text()).then(data => { container.innerHTML = data; });
    }
    ...
    function loadUserEditor(id) {
        ...
        const params = new URLSearchParams();
        params.append('um_ajax_get_editor', id);
        params.append('can_edit', '1');
        fetch('modules/um_sync_worker.php', { method: 'POST', body: params })
        .then(res => res.text()).then(data => { container.innerHTML = data; });
    }
    ...
    function loadNewUserForm() {
        ...
        const params = new URLSearchParams();
        params.append('um_ajax_get_add_form', '1');
        params.append('can_edit', '1');
        fetch('modules/um_sync_worker.php', { method: 'POST', body: params })
        .then(res => res.text()).then(data => { container.innerHTML = data; });
    }
