...
define('IN_CMS', true);    
/**
 * Aldhran Freeshard - Main Index
 * Version: 1.8.0 - ADDED: Discord OAuth2 Support & Domain Integration
 */
...
$timeout_duration = 1800; // 30 Minuten

if (isset($_SESSION['user_id'])) {
    $uid = (int)$_SESSION['user_id'];

    // Update mit Unix-Timestamp (Sekunden)
    $conn->query("UPDATE users SET last_activity = " . time() . " WHERE id = $uid");

    // Prüfen auf Inaktivität (Timeout)
    ...
    $auth_check = $conn->query("SELECT id FROM users WHERE id = $uid AND standing < 5");
...
$unread_count = 0;
if ($currentUserId > 0) {
    $notif_res = $conn->query("SELECT COUNT(id) as cnt FROM spike_notifications WHERE user_id = $currentUserId AND is_read = 0");
    if ($notif_res) {
...
if ($userPriv >= 5 && isset($_POST['toggle_maintenance'])) {
    if (file_exists($lock_path)) { @unlink($lock_path); } 
    else { @file_put_contents($lock_path, 'ACTIVE'); }
...
if ($can_edit && isset($_POST['update_page_content'])) {
    ...
    $stmt_save = $conn->prepare("REPLACE INTO pages (slug, title, content) VALUES (?, ?, ?)");
    $stmt_save->bind_param("sss", $target_slug, $page_title, $page_content);
    
    if ($stmt_save->execute()) {
        header("Location: index.php?p=" . urlencode($target_slug) . "&msg=success"); 
    } else {
        die("Error saving chronic: " . $conn->error);
    }
    exit;
}
...
// NEU: Discord Bridge Abfang-Logik
if ($page_slug === 'discord_callback') {
    $logic_to_include = 'includes/discord_logic.php';
} 
elseif ($page_slug === 'um' && $can_edit) {
    $logic_to_include = 'modules/um_logic.php';
} elseif ($page_slug === 'admin_log' && $can_edit) {
    $logic_to_include = 'modules/admin_log_logic.php';
} else {
